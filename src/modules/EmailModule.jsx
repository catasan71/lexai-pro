...
export default function EmailModule({ showToast }) {
    ...
    <div style={{ overflowY:"auto",padding:isMobile?16:24,flex:1 }}>
      ...
      <div style={{ marginBottom:14 }}>
        ...
        <div style={{ display:"flex",flexWrap:"wrap",gap:7 }}>
          {tones.map(function(t){ return <button key={t.v} onClick={()=>upd("tone",t.v)} style={{ background:form.tone===t.v?"#818cf815":"none",border:"1px solid "+(form.tone===t.v?"#818cf8":"#1e293b"),borderRadius:100,padding:"6px 13px",color:form.tone===t.v?"#818cf8":"#64748b",fontFamily:"'Inter',sans-serif",fontSize:12,cursor:"pointer" }}>{t.l}</button>; })}
        </div>
      </div>
      ...
      <button onClick={generate} disabled={loading} style={{ width:"100%",marginTop:18,background:"linear-gradient(135deg,#818cf8,#6ee7b7)",border:"none",borderRadius:11,padding:13,color:"#070d1a",fontFamily:"'Space Grotesk',sans-serif",fontWeight:700,fontSize:14,cursor:loading?"not-allowed":"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8,opacity:loading?.7:1 }}>
  ...
  var resultContent = (
    <div style={{ overflowY:"auto",padding:isMobile?16:24,flex:1 }}>
      {isMobile && <button onClick={()=>setPanel("form")} style={{ background:"#1e293b",border:"none",borderRadius:8,padding:"8px 14px",color:"#94a3b8",cursor:"pointer",fontSize:13,fontFamily:"'Inter',sans-serif",marginBottom:16,display:"flex",alignItems:"center",gap:6 }}>← Configurare</button>}
      {result ? (
        <div>
          ...
          <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10,flexWrap:"wrap",gap:8 }}>
            ...
            <div style={{ display:"flex",gap:8 }}>
              <button onClick={()=>{navigator.clipboard.writeText(result.corp);showToast("Email copiat!","success");}} style={{ background:"#1e293b",border:"1px solid #334155",borderRadius:8,padding:"6px 12px",color:"#e2e8f0",fontSize:12,cursor:"pointer",fontFamily:"'Space Grotesk',sans-serif",fontWeight:600 }}>📋 Copiază</button>
              <button onClick={()=>setResult(null)} style={{ background:"#1e293b",border:"1px solid #334155",borderRadius:8,padding:"6px 12px",color:"#e2e8f0",fontSize:12,cursor:"pointer",fontFamily:"'Space Grotesk',sans-serif",fontWeight:600 }}>🔄 Nou</button>
            </div>
          </div>
          <div style={{ background:"#0a1628",border:"1px solid #1e293b",borderRadius:12,padding:16,whiteSpace:"pre-wrap",fontSize:13,color:"#94a3b8",lineHeight:1.8,maxHeight:400,overflowY:"auto",fontFamily:"'Inter',sans-serif" }}>{result.corp}</div>
        </div>
      ) : (
        <div style={{ display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:240,textAlign:"center",padding:20 }}>
          ...
          <div style={{ fontFamily:"'Space Grotesk',sans-serif",fontWeight:700,fontSize:16,color:"#475569",marginBottom:8 }}>Niciun email generat</div>
